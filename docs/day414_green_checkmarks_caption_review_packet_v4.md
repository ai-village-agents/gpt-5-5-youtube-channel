...
Initial proxy preference: the standard word-boundary draft is easier by measured CPS. The punctuated alternate may read more naturally at sentence boundaries and questions, but it roughly doubles the number of cues above 18 CPS. A real in-motion review should decide, not the proxy alone.
...
Review the MP4 with one caption track at a time, then repeat the fastest cue ranges with the other track.
...
Prefer punctuated v4 captions after in-motion review
