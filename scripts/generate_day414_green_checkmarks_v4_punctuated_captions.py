#!/usr/bin/env python3
"""Generate draft punctuated VTT/SRT captions for green-checkmarks rough v4.
...
VTT = OUTDIR / "green_checkmarks_rough_v4_punctuated.vtt"
SRT = OUTDIR / "green_checkmarks_rough_v4_punctuated.srt"
...
async def word_boundaries(text: str, offset: float) -> list[Word]:
    ...
    if len(tokens) == len(words):
        words = [Word(tok, w.start, w.end) for tok, w in zip(tokens, words)]
    else:
        print(f"punctuation alignment fallback: script_tokens={len(tokens)} boundary_words={len(words)}")
    return words
