...
OUT = ROOT / "production" / "day413_thinking_partner_rough_v1"
SLIDES = OUT / "slides"
AUDIO = OUT / "audio"
CLIPS = OUT / "clips"
FRAMES = OUT / "frames"
FINAL = OUT / "thinking_partner_rough_v1.mp4"
REPORT = OUT / "scene_timings_v1.txt"
CONTACT = ASSETS / "rough_render_contact_sheet_v1.png"
CONTACT_360 = ASSETS / "rough_render_contact_sheet_v1_360p.png"
FRAME_EXPORT = ASSETS / "rough_render_frames_v1"
...
def draw_header(draw: ImageDraw.ImageDraw, num: int, title: str) -> None:
    ...
    draw.text((1610, 88), "rough render v1", font=font(25), fill=MUTED)
...
def build_contact_sheet(frame_paths: list[Path], timings: list[tuple[int, float, float]]) -> None:
    ...
    d.text((42, 24), "Day 413 rough render v1 contact sheet", font=font(29, True), fill=WARM)
...
def export_review_frames(timings: list[tuple[int, float, float]]) -> None:
    ...
    for num, start, end in timings:
        ...
        raw = FRAMES / f"rough_scene_v1_{num:02d}.png"
        extract_frame_at(t, raw)
        exported = FRAME_EXPORT / f"rough_scene_v1_{num:02d}.png"
...
async def main() -> None:
    ...
    lines = [
        "Day 413 thinking-partner rough render v1",
