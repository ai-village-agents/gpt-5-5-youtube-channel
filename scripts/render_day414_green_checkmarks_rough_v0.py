#!/usr/bin/env python3
"""Render a local rough cut of the Day 414 green-checkmarks candidate.

This is a rough production proof, not a publish-ready renderer. It uses the v1
script, static visual proof frames, Edge TTS narration, exact audio-duration
still-image clips, and faststart MP4 assembly. The resulting MP4 is ignored by
git under production/ and still requires full watch/listen and caption review.
"""
...
OUT = ROOT / "production" / "day414_green_checkmarks_rough_v0"
AUDIO = OUT / "audio"
CLIPS = OUT / "clips"
FINAL = OUT / "green_checkmarks_rough_v0.mp4"
...
VISUALS = {
    ...
    "07": ASSETS / "green_07_final_checklist.png",
}
...
def make_clip(segment: Segment, audio: Path) -> tuple[Path, float]:
    ...
    # Use exact audio duration for the still-image clip so final concat does not
    # need a global -t and does not cut the last narration.
    run([
        "ffmpeg", "-nostdin", "-y",
...
async def main() -> None:
    ...
    lines = ["Green-checkmarks rough render v0", "", f"final_duration {final_dur:.3f}", "", "Segments:"]
    for key, title, start, end, clip_dur, audio_dur, words in timings:
        lines.append(f"{key}  {start:07.3f}  {end:07.3f}  clip={clip_dur:06.3f}  audio={audio_dur:06.3f}  words={words:3d}  {title}")
    lines.append("")
