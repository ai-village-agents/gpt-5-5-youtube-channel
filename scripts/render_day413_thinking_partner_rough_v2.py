...
v4 narration, pairs each scene with the current visual proof/mockup assets,
...
OUT = ROOT / "production" / "day413_thinking_partner_rough_v2"
SLIDES = OUT / "slides"
AUDIO = OUT / "audio"
CLIPS = OUT / "clips"
FRAMES = OUT / "frames"
FINAL = OUT / "thinking_partner_rough_v2.mp4"
REPORT = OUT / "scene_timings_v2.txt"
CONTACT = ASSETS / "rough_render_contact_sheet_v2.png"
CONTACT_360 = ASSETS / "rough_render_contact_sheet_v2_360p.png"
FRAME_EXPORT = ASSETS / "rough_render_frames_v2"
...
SCENE_VISUALS = {
    ...
    4: ASSETS / "scene04_gauge_proof_v2.png",
...
def draw_header(draw: ImageDraw.ImageDraw, num: int, title: str) -> None:
    ...
    draw.text((1610, 88), "rough render v2", font=font(25), fill=MUTED)
...
def build_contact_sheet(frame_paths: list[Path], timings: list[tuple[int, float, float]]) -> None:
    ...
    d.text((42, 24), "Day 413 rough render v2 contact sheet", font=font(29, True), fill=WARM)
...
def export_review_frames(timings: list[tuple[int, float, float]]) -> None:
    ...
    for num, start, end in timings:
        ...
        raw = FRAMES / f"rough_scene_v2_{num:02d}.png"
        extract_frame_at(t, raw)
        exported = FRAME_EXPORT / f"rough_scene_v2_{num:02d}.png"
...
async def main() -> None:
    ...
    lines = [
        "Day 413 thinking-partner rough render v2",
