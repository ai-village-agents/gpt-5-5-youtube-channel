"""Generate caption-band-clear visual polish assets for green-checkmarks scenes 03-06.

Local visual experiment only: these PNGs support review of whether moving lower
callouts out of the caption band improves readability. They are not upload
...
def base(title, scene):
    ...
    d.text((1600, 92), "visual proof v10", font=F_TINY, fill=MUTED)
    return img, d
...
def frame03():
    ...
    return save(img, "green_03_fresh_base_caption_band_v10.png")
...
def frame04():
    ...
    return save(img, "green_04_right_diff_caption_band_v10.png")
...
def frame05():
    ...
    rounded(d, (220, 270, 1700, 800), radius=32, fill=PANEL)
    d.text((310, 335), "Checked", font=F_MED, fill=GREEN)
    d.text((980, 335), "Still ask", font=F_MED, fill=PINK)
    ...
    for i, (left, right) in enumerate(rows):
        ...
        d.line((300, yy+70, 1620, yy+70), fill="#263044", width=2)
    return save(img, "green_05_uncovered_risk_caption_band_v10.png")
...
def frame06():
    ...
    for left, right, color in chips:
        rounded(d, (360, yy, 1560, yy+130), radius=30, fill=PANEL, outline=color, width=3)
        d.text((430, yy+38), left, font=F_MED, fill=color)
        d.text((850, yy+38), right, font=F_MED, fill=TEXT)
        yy += 170
    return save(img, "green_06_signals_not_verdicts_caption_band_v10.png")
...
def contact_sheet(paths):
    ...
    d.text((40, 28), "Green checkmarks caption-band visual polish v10", font=font(30, True), fill=TEXT)
    ...
    path = OUT / "green_checkmarks_caption_band_v10_contact_sheet.png"
    sheet.save(path)
    return path
